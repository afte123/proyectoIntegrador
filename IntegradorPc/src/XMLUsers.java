...
public class XMLUsers {
	...
	public void addUser(String nombre, String apellido,  String contra, String nickName) {
		...
		for (int i = 0; i < variantes.length; i++) {
			...
			if (variantes[i].getString("usuario").equals(nickName)) {
		...
		// Si no está...
		if (!yaEsta) {
			// Que se cree una línea de código con el usuario y contraseña
			// ingresados
			XML hijo = app.parseXML("<user usuario=\"" + nickName + "\" contrasena=\"" + contra + "\"></user>");
			// Se agrega el hijo
			users.addChild(hijo);
